...
export default class FeesFarmingApi {
  ...
  public async stake(
    ...
    const addOperator = FeesFarmingApi.updateOperatorTransaction(stakedTokenContractAddress, account, farmContractAddress);
    const stake = FeesFarmingApi.stakeOperation(farmContractAddress, amount.toString(10));
  ...
  public async stakeAll(newStakes: NewStake[], account: string): Promise<string> {
    ...
    const operators = validNewStakes.map((stake): WalletParamsWithKind => FeesFarmingApi.updateOperatorTransaction(stake.farmStakedToken, account, stake.contract));
    const stakes = validNewStakes.map((stake): WalletParamsWithKind => FeesFarmingApi.stakeOperation(stake.contract, new BigNumber(stake.amount).shiftedBy(stake.stakeDecimals).toString(10)));
  ...
  public async unstakeAll(stakingBalances: ContractBalance[]): Promise<string> {
    ...
    }).map((stake): WalletParamsWithKind => FeesFarmingApi.unstakeOperation(stake.contract, stake.balance));
  ...
  public async claimAll(claimBalances: FarmConfigWithClaimBalances[]): Promise<string> {
    ...
    }).map((claim): WalletParamsWithKind => FeesFarmingApi.claimOperation(claim.farmContractAddress)));
